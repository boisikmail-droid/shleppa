...
При первом запуске автоматически выполняются миграции и импорт словаря (~7200 слов: 12 категорий × 6 уровней × 100).

Поток: **приветствие** → **настройка** (команды 2–4 → слова/время → сложности и категории) → игра.
